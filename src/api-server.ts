...
export class ApiServer {
    ...
    constructor(port: number | undefined = 7000) {
        ...
        Server.buildServices(this.apiRouter, ...publicApi, ...adminApi, ...licensedApi);

        // TODO: enable for Swagger generation error
        // Server.loadServices(this.app, 'controllers/*', __dirname);
        Server.swagger(this.app, "./build/swagger.json", "/api-docs", `${config.hostUrl.host}/api`, [config.hostUrl.scheme]);
    ...
    public start(): Promise<any> {
        return new Promise<any>((resolve, reject) => {
            this.server = this.app.listen(config.port, (err: any) => {
                ...
                console.log(`Listening to ${config.hostUrl.scheme}://${config.hostUrl.host}/`);

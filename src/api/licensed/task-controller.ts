...
import {
  TaskServiceStatus,
  TaskServiceRunner
} from "./models/task-service-status";
import { state } from "../../state";
import { validateLicenseKey } from "./license-validator";
import {
  TaskRequest,
  TaskRequestStatus,
  TaskRequestStatusType
} from "./models/task-request";
...
import {
  ...
  getTaskRunnerConfigByName
} from "./is-supported-task";
...
export class TaskController {
  ...
  public async executeTask(
  ...
  ): Promise<TaskRequestStatus> {
    // if task runner is not ready it should be soon
    if (!state.isTaskRunnerReady) {
      logger.info(
        `Task Runner not ready...delaying ${config.autoScaleDelay} ms`
      );
    ...
    if (!state.isTaskRunnerReady) {
      logger.warn(
        `Task Runner still not ready! Rejecting task for service: ${
          taskRequest.service
        }`
      );
      return new TaskRequestStatus(
        TaskRequestStatusType.REJECTED,
        taskRequest.trackingInfo
      );
    } else {
      ...
      if (!isSupportedTask(taskRequest)) {
        logger.warn(
          `Task Runner does not support service: ${taskRequest.service}`
        );
        return new TaskRequestStatus(
          TaskRequestStatusType.UNSUPPORTED,
          taskRequest.trackingInfo
        );
      }
      ...
      if (!state.hasAvailableCapacity(taskWeight)) {
        logger.info(
          `Task Runner does not current have enough capacity ${
            state.currentCapacity
          } for task with weight: ${taskWeight}`
        );
        logger.info(
          `Waiting ${
            config.autoScaleDelay
          } ms to see if more capacity becomes available`
        );
        await delay(config.autoScaleDelay);
      }
      ...
      if (!state.hasAvailableCapacity(taskWeight)) {
        logger.warn(
          `Task Runner still does not current have enough capacity ${
            state.currentCapacity
          } for task with weight: ${taskWeight}`
        );
        ...
        logger.info(
          `Task Runner has available capacity ${
            state.currentCapacity
          } and has ${
            state.activeTasks.length
          } in process. Adding task to active task list.`
        );
        const taskServiceRunner = new TaskServiceRunner(
          getTaskRunnerConfigByName(taskRequest.service)
        );
        ...
        task.start();
        return new TaskRequestStatus(
          TaskRequestStatusType.ACCEPTED,
          taskRequest.trackingInfo,
          task.id,

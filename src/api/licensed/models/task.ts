...
export class Task extends TaskRequest {
  ...
  set status(val: TaskStatusType) {
    logger.info(
      `Changing status for task ${this.id} from ${this._status} to ${val}`
    );
    this._status = val;
  }
    ...
    try {
      ...
      if (resp.data.cancel) {
        if (cancellableStates.indexOf(this.status) > -1) {
          ...
          logger.warn(
            `Unable to cancel task ${this.id} because its current state ${
              this.status
            } is not cancellable`
          );
        }
  ...
  async execute(): Promise<any> {
    ...
    logger.info(
      `Task ${this.id} is loading module: ${this.serviceRunner.modulePath}`
    );
    this.workerFarm = new Farm(
      workerFarmOptions,
      this.serviceRunner.modulePath
    );
    ...
    return new Promise((resolve, reject) => {
      try {
        if (task.status === TaskStatusType.CANCELLED) {
          logger.info(
            `Not trying to end worker farm for task ${
              task.id
            } because task has been cancelled`
          );
          resolve();
          return;
        }
        ...
        task.workerFarm.end((err, result) => {
          if (err) {
            logger.error(
              `[TASK-1011] Error ending worker farm for task ${task.id}`
            );
            logErrorObjectAtAppropriateLevel(err);
            reject(err);
          } else {
        ...
        logger.error(
          `[TASK-1012] Error destroying worker farm for task ${task.id}`
        );
        logErrorObjectAtAppropriateLevel(err);
        reject(err);
      }
  ...
  async processResult(result: any, isError: boolean = false): Promise<any> {
    ...
    if (this.status === TaskStatusType.CANCELLING) {
      logger.info(
        `Skipping process results for task ${
          this.id
        } because it's being cancelled`
      );
      return;
    }
    ...
    try {
      ...
      logger.error(
        `[TASK-1005] Error stopping worker farm for task ${this.id}`
      );
      logErrorObjectAtAppropriateLevel(err);
      finalResult = err;
      isError = true;
    ...
    try {
      if (!isError) {
        ...
        if (outputFiles.length > 0) {
          logger.info(
            `Uploading output ${outputFiles.length} files for task ${this.id}`
          );
        } else {
        ...
        logger.warn(
          `Skipping uploading output files for task ${
            this.id
          } because it has errored!`
        );
      }
    } catch (err) {
      logger.error(
        `[TASK-1006] Error uploading output files for task ${this.id}`
      );
      logErrorObjectAtAppropriateLevel(err);
      finalResult = err;
      isError = true;
  ...
  async uploadOutputFile(outputFilePath: string): Promise<any> {
    ...
    try {
      ...
      logger.info(
        `[TASK-1008] Received file drop instructions for output file: ${relFilePath} for task ${
          this.id
        }.`,
        resp.data
      );
      ...
      return axios
        ...
        .then(res => {
          ...
          logger.info(
            `Successfully posted file ${outputFilePath} to ${postUrl}`
          );
          return res;
        })
        .catch(err => {
          logger.error(
            `[TASK-1009] Error posting file ${outputFilePath} to ${postUrl}`
          );
          logErrorObjectAtAppropriateLevel(err);
          if (err.response) {
      ...
      logger.error(
        `[TASK-1010] Error uploading output file ${relFilePath} for task ${
          this.id
        }`
      );
      logErrorObjectAtAppropriateLevel(err);

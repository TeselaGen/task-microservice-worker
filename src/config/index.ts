...
const taskRunnerManifestPath = path.resolve(
  __dirname,
  "../../task-runner-manifest.json"
);
const taskRunnerConfigPath = path.resolve(
  __dirname,
...
if (
  ...
  console.warn(
    `Running in a production environment but neither HEROKU_APP_NAME or HOST_URL environment variables are set!`
  );
  console.warn(
    `If running on Heroku run command: 'heroku labs:enable runtime-dyno-metadata -a <app_name>' to enable the HEROKU_APP_NAME variable`
  );
}
